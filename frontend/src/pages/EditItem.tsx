...
export default function EditItem() {
  ...
  useEffect(() => {
    ...
    pantryApi.get(id)
      .then(item => {
        ...
        setUnit(item.unit || 'cái');
        if (item.expiryDate) {
          setExpiryDate(new Date(item.expiryDate).toISOString().slice(0, 10));
        }
        setLocation(LOCATION_LABELS[item.location] ?? 'Tủ mát');
        if (item.categoryId) setCategoryId(item.categoryId);
        if (item.imageUrl) setImageUrl(item.imageUrl);
        
        // Try to fetch food details if it has a foodId
        if (item.foodId) {
          catalogApi.searchFoods({ q: item.name }).then(foods => {
      ...
      .catch(err => {
        showAlert(err instanceof Error ? err.message : 'Không tải được chi tiết thực phẩm.');
        navigate('/pantry');
      })
      .finally(() => {
        setLoading(false);
  ...
  const handleSubmit = async (e?: React.FormEvent | React.MouseEvent) => {
    e?.preventDefault();
    if (!id || !itemName.trim() || saving || uploadingImage) return;
    if (!expiryDate) {
    ...
    try {
      ...
      showAlert('Đã cập nhật thực phẩm thành công!');
      navigate('/pantry');
    } catch (err) {
      showAlert(err instanceof Error ? err.message : 'Không cập nhật được thực phẩm.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="bg-background text-on-background font-body-md antialiased h-screen flex flex-col">
      <header className="shrink-0 bg-surface dark:bg-surface-dim border-b border-outline-variant z-40 px-margin-mobile h-nav-height flex items-center justify-between shadow-sm">
        ...
        <h1 className="font-headline-sm text-headline-sm font-bold text-on-surface">Chỉnh sửa thực phẩm</h1>
        <button 
