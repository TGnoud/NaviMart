...
% =====================================================================
\section{Kiểm thử tích hợp trên môi trường triển khai (live black-box)}
\label{sec:int-live}
...
(Bảng~\ref{tab:intlive}). Toàn bộ pass khi chạy ngày 2026-06-20 đối với bản triển khai hiện hành.

\begin{table}[h]
\centering
\caption{Danh mục kiểm thử tích hợp live (black-box)}
\label{tab:intlive}
\begin{tabularx}{\linewidth}{|l|l|Y|}
\hline
\rowcolor{headerblue}
\thead{Khu vực} & \thead{Mã} & \thead{Kịch bản \& khẳng định} \\
\hline
Frontend (Vercel) & INT-WEB-001 & \code{GET /} trả 200 và chứa khung SPA (\code{<title>NaviMart</title>}, \code{<div id="root">}). \\
\hline
Frontend & INT-WEB-002 & Deep link \code{/home} không 404 --- rewrite SPA trả về \code{index.html} cho định tuyến phía client. \\
\hline
Frontend & INT-WEB-003 & Bundle tĩnh \code{/assets/index-*.js} tải được, đúng \code{content-type} JavaScript. \\
\hline
Backend & INT-API-001 & \code{GET /api/health} trả \code{status:"ok"}, \code{service:"navimart-backend"}. \\
\hline
Auth$\to$Users & INT-AUTH-001 & Đăng ký tài khoản mới 201, tự tạo gia đình (\code{activeFamilyId}), trả cặp access/refresh token. \\
\hline
Auth & INT-AUTH-002 & Đăng nhập lại bằng cùng thông tin 200, đúng \code{user.id}. \\
\hline
Auth & INT-AUTH-003 & \code{GET /api/users/me} với token hợp lệ trả đúng hồ sơ (200). \\
\hline
Auth & INT-AUTH-004 & \code{POST /api/auth/refresh} xoay và phát cặp token mới (200). \\
\hline
Catalog & INT-CAT-001 & \code{GET /api/catalog/foods?q=ca} có token trả danh sách món (200). \\
\hline
Catalog & INT-CAT-002 & \code{categories} và \code{units} truy cập được khi đã xác thực (200). \\
\hline
Pantry & INT-PAN-001 & Tạo item kho 201, trả về định danh. \\
\hline
Pantry & INT-PAN-002 & Item vừa tạo xuất hiện trong danh sách kho (200). \\
\hline
Pantry & INT-PAN-003 & \code{GET /api/pantry/:id} đọc đúng item thuộc sở hữu (200). \\
\hline
Pantry & INT-PAN-004 & Xóa item để dọn dẹp production (200/204). \\
\hline
Recipes & INT-RCP-001 & \code{GET /api/recipes} trả kết quả (200). \\
\hline
Recipes & INT-RCP-002 & \code{GET /api/recipes/suggestions} xếp hạng theo kho (200). \\
\hline
Family & INT-FAM-001 & \code{GET /api/family} trả gia đình tự tạo kèm danh sách thành viên (200). \\
\hline
Shopping list & INT-SL-001..003 & Tạo danh sách 201 (\code{status:active}); xuất hiện trong danh sách + đọc theo id 200; xóa dọn dẹp 200/204. \\
\hline
Meals & INT-MEAL-001..002 & Tạo bữa ăn 201; đọc theo id 200 rồi xóa 200/204. \\
\hline
Notifications & INT-NTF-001 & \code{GET /api/notifications} trả mảng (200). \\
\hline
Reports & INT-RPT-001 & \code{GET /api/reports/dashboard} kèm khoảng ngày tổng hợp đúng (200, có \code{range}/\code{shopping}). \\
\hline
AI Chef & INT-AI-001 & \code{GET /api/ai-chef/status} báo cấu hình (200, \code{configured} boolean). \\
\hline
CORS & INT-CORS-001 & Preflight từ origin Vercel được chấp nhận (ACAO khớp origin, ACAC \code{true}). \\
\hline
Logout & INT-LOGOUT-001 & Sau \code{logout}, refresh token cũ bị thu hồi $\to$ 401. \\
\hline
\end{tabularx}
\end{table}

% =====================================================================
\section{Kiểm thử bảo mật trên môi trường triển khai (live black-box)}
\label{sec:sec-live}
...
Bộ bảo mật gồm 20 testcase (Bảng~\ref{tab:seclive}), tất cả đều là cổng cứng (pass/fail). Toàn bộ pass
ngày 2026-06-20.
...
Toàn bộ bộ kiểm thử ở trạng thái xanh. Backend chạy 293 testcase đơn vị/API trên 37 bộ test cùng 32 testcase
đầu-cuối trên 4 bộ test, tất cả đều pass, không có lỗi hay thất bại nào; thời gian chạy phần đơn vị/API
khoảng sáu giây. Trong đợt này cũng đã sửa ba testcase đầu-cuối bị ``stale'' (kỳ vọng cũ trước khi chuẩn hóa
...
cũng pass toàn bộ (chạy ngày 2026-06-20, khoảng 40 giây gồm cả thời gian cold start của Render). Frontend chạy
...
Độ phủ tổng thể trên toàn bộ \code{src} đạt 61,91\% câu lệnh và 52,70\% nhánh (đo bằng \code{npm run test:cov}
ngày 2026-06-20). Khi xét riêng theo module (Bảng~\ref{tab:cov}), các module nghiệp vụ cốt lõi --- xác thực,
gia đình, kho, công thức, bữa ăn, danh sách mua, báo cáo, danh mục, thông báo, người dùng và AI Chef --- đều
đạt độ phủ cao (trên dưới 80\%), phản ánh đúng chiến lược ``ưu tiên logic nghiệp vụ''. Các tệp cấu hình, seed
dữ liệu và cổng realtime được phủ chủ yếu qua kiểm thử đầu-cuối nên kéo tỷ lệ câu lệnh chung xuống.
...
\hline
ai-chef & 89,42 & Đạt (service 100\%, controller 100\%, DTO) \\
\hline
notifications & 85,98 & Đạt (service + API + nhắc hạn) \\
\hline
users & 83,33 & Đạt (service + API) \\
\hline
catalog & 82,75 & Đạt (service + API) \\
\hline
reports & 82,55 & Đạt (service + API) \\
\hline
pantry & 81,46 & Đạt \\
\hline
families & 79,26 & Đạt \\
...
meals & 77,28 & Đạt \\
\hline
auth & 70,76 & Đạt (service cao; controller + chiến lược Passport) \\
\hline
shopping-lists & 69,23 & Đạt \\
\hline
admin & 47,54 & Service phủ cao; controller phủ qua E2E \\
\hline
realtime & 8,88 & Phủ qua E2E \\
\hline
uploads & 0,00 & Phủ qua kiểm thử thủ công \\
\hline
\end{tabularx}
\end{table}
...
lệch nghiệp vụ. Thứ hai, các tệp cấu hình, seed dữ liệu, cổng realtime và DTO được phủ qua kiểm thử đầu-cuối
và kiểm thử live thay vì kiểm thử đơn vị, vì đó là nơi hành vi thật của chúng được kiểm chứng đầy đủ nhất.
...
\hline
1 & \code{cd backend \&\& npm run test} & 37 bộ / 293 testcase pass. \\
\hline
